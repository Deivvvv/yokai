...
public class WorldEditor : MonoBehaviour
{
    ...
    void NewPoint()
    {
        ...
        if(i != -1)
        {
            ...
            selectObject = i;
           // GamePoint point = GamePoints[i];
          //  sliderPath = new List<int>(new int[point.Point.Count]);
          //  for (int j = 0; j < point.Point.Count; j++)
    ...
    void SetNewPostion()
    {
        ...
        //проверка что пути не пересекаютс€
        if (crossLook)
        {
            WarningCrossWindow.SetActive(true);
        }
        else
            //сделать оео подвержден€и на основе uilook
